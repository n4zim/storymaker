...
fn startup(mut commands: Commands, asset_server: Res<AssetServer>) {
  ...
  let map_file = read_to_string("island1/map/island1.tmx").unwrap();

  let map: TiledMap = from_str(&map_file).unwrap();
  ...
  App::new()
    .add_plugins(
      DefaultPlugins
        ...
        .set(AssetPlugin {
          watch_for_changes: ChangeWatcher::with_delay(Duration::from_millis(
            200,
          )),
          ..default()
        }),
